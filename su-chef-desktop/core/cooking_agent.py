...
class CookingAgent:
    def __init__(self, test_microphone=True):
        ...
        # Event-driven components (minimal addition)
        self.state = CookingState.IDLE
...
"""
    ...
    # ===== EVENT-DRIVEN INTERFACE (minimal addition) =====
    
    def set_ui_callbacks(self, **callbacks):
        """Set UI callback functions (dummy for compatibility)"""
        pass
    
    def start_session(self) -> bool:
        """Start non-blocking cooking session"""
        if not self.load_recipe():
            return False
        self.state = CookingState.READY
        return True
    
    def send_command(self, command: str):
        """Send command for processing - use existing CLI methods"""
    ...
    def speak_current_step(self):
        """Speak current step"""
        if 0 <= self.current_step < len(self.recipe_steps):
            # Handle both string and dict formats
            if isinstance(self.recipe_steps[self.current_step], dict):
                step_content = self.recipe_steps[self.current_step]['text']
            else:
                step_content = self.recipe_steps[self.current_step]
            step_text = f"Step {self.current_step + 1}: {step_content}"
            print(f"🔊 {step_text}")
            self.speak(step_text)
    
    def stop_session(self):
        """Stop cooking session"""
        self.state = CookingState.STOPPED
        self.is_interrupted = True
